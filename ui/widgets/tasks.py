...
class TasksWidget(WidgetBase):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        app: TokyoNotes | None = self.app
        assets_dir: Path | None = None
        if app is not None:
            assets_dir = app.base_dir / "assets"

        self._show_completed = bool(app and app.cfg.get("show_completed", True))
        ...
        if assets_dir:
            icon = Gtk.Image.new_from_file(str(assets_dir / "dashboard" / "new.svg"))
            icon.set_pixel_size(16)
            self.quick_add_btn.set_child(icon)
        ...
        self._content.append(filter_box)

        self.active_filter: str = "all"
        self.update_active_filter("all")
    ...
    def _build_quick_add_popover(self) -> None:
        app: TokyoNotes | None = self.app
        assets_dir = (app.base_dir / "assets") if app else None
        ...
        if assets_dir:
            cal_icon = Gtk.Image.new_from_file(
                str(assets_dir / "dashboard" / "calendar.svg")
            )
    ...
    def _refresh_quick_add_notes(self) -> None:
        ...
        if app is None:
            return
        notes_set = set(app.notes_manager.get_notes())
        notes_set.discard("Inbox")
    ...
    def _on_quick_add_submit(self, *args) -> None:
        ...
        app = self.app
        if not text or app is None or not hasattr(app, "on_quick_add_task"):
            return
        ...
        app.on_quick_add_task(text, note_name, self._quick_add_deadline)
        self._quick_add_entry.set_text("")
    ...
    def _refresh(self, filter_type: str) -> None:
        app = self.app
        if app is None:
            return
        checkboxes = app.notes_manager.get_all_checkboxes(exclude=app.cfg.archived)
        self.populate(checkboxes, filter_type)
    ...
    def _on_completed_toggled(self, switch: Gtk.Switch, *_) -> None:
        ...
        self._refresh(self.active_filter)
        self._update_filter_indicator()
    ...
    def populate(self, checkboxes: list[dict[str, Any]], filter_type: str) -> int:
        ...
        if not self._temp_show_completed:
            self._show_completed = bool(app.cfg.get("show_completed", True))
    ...
    def _show_empty(self, filter_type: str) -> None:
        ...
        app = self.app
        base_dir = app.base_dir if app else None
        widget = create_empty_state_widget(msg, base_dir)
    ...
    def _filter(
        ...
        _WEEK_START, _WEEK_END = get_week_boundaries(
            bool(app and app.cfg.get("start_week_on_sunday", True))
        )
    ...
    def _make_date_header(
        ...
        show_rings = bool(app and app.cfg.get("show_progress_rings", True))
        if progress is not None and show_rings:
    ...
    def _make_row(self, cb: dict[str, Any]) -> Gtk.ListBoxRow:
        ...
        if time_str:
            ...
            deadline_gesture.connect(
                "pressed",
                lambda *a, _cb=cb: self._on_deadline_click(_cb, a[2], a[3]),
            )
        ...
        row._checkbox_handler_id = checkbox.connect(
            "toggled",
            lambda btn, _cb=cb: self._on_checkbox_toggled(_cb, btn.get_active()),
        )
    ...
    def _on_checkbox_toggled(self, cb: dict, checked: bool) -> None:
        app = self.app
        if app is None:
            return
        app.on_dashboard_checkbox_toggled(cb, checked)

    def _on_deadline_click(self, cb: dict, x: float, y: float) -> None:
        app = self.app
        if app is None:
            return
    ...
    def _show_snooze_popover(
        ...
        for _label, action_name, dl in presets:
            ...
            action.connect(
                "activate",
                lambda *a, _dl=dl: self._apply_snooze(self._snooze_cb, _dl),
            )
    ...
    def _apply_snooze(self, cb: dict[str, Any], new_dl: str | None) -> None:
        ...
        if app is None:
            return
        app.handle_snooze(cb["note"], cb["line"], new_dl, cb.get("text", ""))

    def _snooze_pick_date(self, cb: dict[str, Any]) -> None:
        self._on_deadline_click(cb, 0, 0)
    ...
    def update_checkbox(self, note_name: str, line_num: int, checked: bool) -> bool:
        ...
        if target_date and target_date in self._header_rows:
            ...
            total = len(rows)
            ring = getattr(header_row, "_progress_ring", None)
            if ring:
